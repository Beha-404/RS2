...
### Lokacija Source Code-a

**Backend:**
- Putanja: `EasyPC.Services/PcService.cs`
- Metode: 
  - `Recommend(int ID)` - glavna metoda za generisanje preporuka (linije 190-258)
  - `CalculateSimilarityScore(Database.PC target, Database.PC candidate)` - izračunavanje similarity score-a (linije 260-318)

**API Endpoint:**
- Putanja: `EasyPC.API/Controllers/PcController.cs`
- Endpoint: `GET /api/pc/recommend/{id}`

**Frontend (Mobile):**
- Putanja: `UI/easy_pc_mobile/lib/services/pc_service.dart`
- Metoda: `getRecommendations(int pcId)`
- Widget: `UI/easy_pc_mobile/lib/widgets/dialog/pc_details_dialog.dart` (prikaz preporuka)
...
1. **Collaborative Filtering** - dodati analizu ponašanja korisnika
2. **Hybrid Approach** - kombinovati Content-Based i Collaborative
3. **Machine Learning** - naučiti optimalne weight-ove iz podataka
4. **A/B Testing** - testirati različite weight konfiguracije
